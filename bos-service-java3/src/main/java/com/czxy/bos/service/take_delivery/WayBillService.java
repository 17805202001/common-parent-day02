...
/**
 * Created by liangtong on 2018/9/26.
 */
@Service
@Transactional
public class WayBillService {
    ...
    /**
     * 如果运单id存在，表示为快速录入的数据，此处进行的是更新操作
    ...
    public Page<ESWayBill> pageQuery(ESWayBill wayBill , Integer page , Integer rows){
        ...
        if(StringUtils.isNotBlank( wayBill.getSendAddress() )){
            //模糊查询，默认各个分词之间使用or关系。通过Operator可以设置成and关系，同时可以通过minimumShouldMatch控制搜索结果优化。
            //queryBuilder.must( QueryBuilders.matchQuery("sendAddress" , wayBill.getSendAddress()).operator( Operator.AND));
            //queryBuilder.must( QueryBuilders.matchQuery("sendAddress" , wayBill.getSendAddress()).operator( Operator.AND).minimumShouldMatch("75%")  );

            queryBuilder.must( QueryBuilders.matchPhraseQuery("sendAddress" , wayBill.getSendAddress()).slop(2)  );
        }
        ...
        if(StringUtils.isNotBlank( wayBill.getSendProNum())){
            ...
            //queryBuilder.must( QueryBuilders.matchQuery("sendProNum", wayBill.getSendProNum()));
        }
    ...
    /**
     * 封装统计信息，内部只有name和data，可以使用Map，此处使用自定义JavaBean
     * @return
     */
    public List<HighChart> chartWayBill(){
        return this.wayBillMapper.findBySignStatus();
    }
